...
(p9 <- ggplot()+
  ...
  annotate(geom = "text",x=5,y=0.155,hjust=0,vjust=0, parse = TRUE,
           label=as.character(as.expression("***"~~italic(R)^2~"="~0.08)),size=4)+
  # stat_summary(data = df_param_kmax_alpha,
  #              mapping=aes(log(jmax_ww),alpha,group=interaction(species,source)),fun.data=mean_sdl, fun.args = list(mult=1),
  # geom="pointrange", color="blue")+
  mytheme3()+
...
# y_partial_jmax_ww <- remef(alpha_mod, fix = "jmax_ww", ran = "all")
# y_partial_hv <- remef(alpha_mod, fix = "hv", ran = "all")
# y_partial_lb <- remef(alpha_mod, fix = "l_b", ran = "all")
# y_partial_P50 <- remef(alpha_mod, fix = "P50", ran = "all")
# df_param_kmax_alpha <- df_param_kmax_alpha %>% 
#   cbind(partial_iabs = y_partial_iabs) %>% 
#   cbind(partial_jmax_ww = y_partial_jmax_ww) %>% 
#   cbind(partial_hv = y_partial_hv) %>% 
#   cbind(partial_lb = y_partial_lb) %>% 
#   cbind(partial_P50 = y_partial_P50) %>% 
...
ggsave("PLOTS/A_metrics.png", width = 18, height = 13, units = "cm")
...
ggsave("PLOTS/gs_metrics.png", width = 18, height = 13, units = "cm")
# 
# df_a_fix %>% 
#   filter(scheme == "PROFITMAX") %>% 
...
ggsave("PLOTS/partition.png", width = 34, height = 11, units = "cm")
